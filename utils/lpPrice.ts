...
export const getZmbeBnbLpTokenValue = async (
    amount: string
) => {
...
export const getCakeBnbLpTokenValue = async (
    amount: string
) => {
    ...
    // zmbe to bnb
    const amountsOneOut =  await Router.methods.getAmountsOut(ethers.utils.parseUnits(String(tokenOne)),[CONSTANTS.CAKE, CONSTANTS.WRAPPED_BNB]).call()
    // get converted bnb value in usdt
    const amountsTwoOut = await Router.methods.getAmountsOut(amountsOneOut[1],[CONSTANTS.WRAPPED_BNB, CONSTANTS.USDT]).call()
    const bnbToUSDT = ethers.utils.formatUnits(amountsTwoOut[1])
    // console.log(bnbToUSDT, ' <<< zmbe in USDT')
