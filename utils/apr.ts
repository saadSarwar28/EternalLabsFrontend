...
export const getCakePoolApr = async () => {
    ...
    const lpValue = await getCakeBnbLpTokenValue('1')
    let poolLiquidityUsd = new BigNumber(lpValue)

    const cakePerBlock = new BigNumber(await MasterChef.methods.cakePerBlock(true).call())
    ...
    const totalRewardPricePerYear = new BigNumber(cakePrice).times(CONSTANTS.CAKE_PER_BLOCK).times(CONSTANTS.BLOCKS_PER_YEAR)
    const totalStakingTokenInPool = new BigNumber(lpValue).times(new BigNumber(lpStored))
    const apr = totalRewardPricePerYear.div(totalStakingTokenInPool).times(100)
    // console.log(apr.toNumber(), ' <<< apr')
